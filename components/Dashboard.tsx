...
export default async function Dashboard() {
  ...
  // const { data: filteredJob, error } = useSWR(
  //   `https://66afff066a693a95b537a511.mockapi.io/jobs`,
  //   fetcher,
  //   {
  //     refreshInterval: 60000, // Revalidate every 60 seconds
  //     dedupingInterval: 60000, // Prevent refetch within this interval
  //     revalidateOnFocus: true, // Revalidate when window is refocused
  //   }
  // );

  // const postedJobs = filteredJob?.filter((currentUserJob:jobdataType) => currentUserJob.recruiterId === user?.id);

  // get all the available jobs
  
  
  const getJobs = async () => {
    const res = await fetch("https://66afff066a693a95b537a511.mockapi.io/jobs");
