...
const CandidateActivity:React.FC<CandidateActivityProps> = ({currentuserId}) => {
  ...
  const uniqueArrayStatus = Array.from(
    new Set(jobAppliedByCurrentuser.flatMap((item:ArrayStatus) => item?.status))
  );
  return (
        ...
        <Tabs defaultValue="Applied">
          ...
          <div className="">
             {uniqueArrayStatus.map((targetStatus, index) => (
              <TabsContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" key={index} value={targetStatus}>
